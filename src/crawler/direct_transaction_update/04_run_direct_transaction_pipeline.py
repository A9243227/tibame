...
主流程總控：01 →（本次 failed 有資料才）02 → 03。

本檔最重要的責任：同一次流程只決定一次 PIPELINE_DT，並傳給所有子程式。
因此即使流程跨午夜，raw、old_raw_data 歷史去重版本與 audit 都會落在相同 dt=YYYY-MM-DD。
"""
...
def get_pipeline_dt() -> str:
    """優先沿用外部傳入的 PIPELINE_DT；否則以 Asia/Taipei 當日產生。"""
...
def failed_csv_has_data() -> bool:
    ...
    with FAILED_CSV_FILE.open("r", newline="", encoding="utf-8-sig") as f:
...
def print_output_summary(pipeline_dt: str) -> None:
    ...
    prefix = os.getenv("GCS_PREFIX", "direct_transaction").strip("/")
    ...
    print(
        ...
        f"trec_direct_transaction_dedup_report_{compact_dt}.csv",
        flush=True,
    )
...
def main() -> int:
    ...
    # 04 的 02 必須只 retry 本次 01 的 failed.csv；不能被 Cloud Run Job 環境變數誤導去讀歷史 retry。
    base_env.pop("FAILED_CSV_FILE", None)

    print("\n" + "#" * 80, flush=True)
    print("04 主資料流程啟動：01 → 02（條件式）→ 03", flush=True)
    print("#" * 80, flush=True)
    ...
    print("HISTORICAL_DEDUP_PREFIX：", base_env.get("HISTORICAL_DEDUP_PREFIX", "old_raw_data/playwright_trec"), flush=True)
    print("LATEST_DEDUP_PREFIX：", base_env.get("LATEST_DEDUP_PREFIX", "new_raw_data/playwright_trec"), flush=True)
