...
3. 主流程預設只 retry 本次 trec_direct_transaction_raw_failed.csv；同時保留舊 failed_retry.csv。
4. Retry-only 流程可設定 FAILED_CSV_FILE=trec_direct_transaction_raw_failed_retry.csv，只補舊 retry。
5. 筆數 = 0：整頁 data API 失敗，data API 最多重試 3 次；成功後處理本頁每一筆 detail。
6. 筆數 > 0：單筆 detail 失敗，只補該頁指定筆數。
7. detail 成交記錄 <ol></ol> 空白是正常沒有成交記錄，不算失敗、不重試。
8. retry 成功資料立即補進年度 raw、立即上傳 GCS、重建並上傳 all_year。
9. retry 仍失敗資料會保留或加入 failed_retry.csv 並上傳 GCS。
10. 02 不改寫 01 的 failed.csv；failed.csv 仍代表本次 01 的失敗清單。
...
def find_latest_raw_snapshot_dt() -> Optional[str]:
    ...
    retry-only 流程會以這個最新 raw 快照作為
    新快照的基礎，確保不會直接修改舊 dt 資料夾。
    """
    if storage_client is None:
...
def ensure_working_raw_snapshot() -> Optional[str]:
    ...
    - 05 retry-only：從 raw/ 下日期最新的快照複製所有年度到 /tmp，
      改名成本次日期，再以新 dt 快照承接 retry 成功資料。
    """
    ...
    for blob in storage_client.list_blobs(GCS_BUCKET, prefix=source_prefix):
        ...
        blob.download_to_filename(str(destination))
        copied.append(year)
        print(f"05/02 複製年度快照：{year}，{filename} -> {destination.name}")

    if not copied:
        raise FileNotFoundError(f"raw/dt={source_dt} 沒有任何年度 raw CSV")

    # 05 沒有先跑 01；先把完整基礎資料寫成本次 dt 的 raw 快照，
    # retry 成功資料之後只會寫入這個新快照，不會改動舊快照。
...
def download_existing_files_from_gcs() -> None:
    """下載 control 檔，並確保本次 raw 工作快照可用。"""
    print("\n========== 02 準備 GCS 資料 ==========")
    print("GCS_BUCKET：", GCS_BUCKET)
    print("GCS_PREFIX：", GCS_PREFIX)
    print("PIPELINE_DT：", PIPELINE_DT)

    download_blob_if_missing(
        control_blob_name(FAILED_CSV_NAME),
        FAILED_CSV_FILE_DEFAULT,
    )
    download_blob_if_missing(
        control_blob_name(RETRY_FAILED_CSV_NAME),
        RETRY_FAILED_CSV_FILE,
    )
    ensure_working_raw_snapshot()


def upload_changed_raw_and_all_year(year_raw_file: Path) -> None:
    """retry 成功後立即更新本次 raw 快照與 all_year。"""
...
def upload_all_current_raw_snapshot() -> None:
    """上傳本次 dt 下所有年度 raw 與本次 all_year。"""
...
def find_latest_year_raw_file(year: str) -> Path:
    """找到本次工作快照指定年度檔；不存在就建立本次日期的新檔名。"""
    return local_current_year_raw_files().get(
        str(year), local_path(get_year_raw_csv_name(str(year)))
    )


def rebuild_all_year_csv() -> None:
    """all_year = 本次快照每個年度 raw 的合併結果。"""
...
def save_retry_failed_csv(force_create_empty: bool = True) -> None:
    ...
    if storage_client is not None:
        upload_file_to_gcs(
            RETRY_FAILED_CSV_FILE, control_blob_name(RETRY_FAILED_CSV_NAME)
        )
...
def merge_failed_row_values(
    ...
    """
    合併同一個 failed key 的欄位。

    incoming_row 有值時優先使用 incoming_row；空白欄位保留原本資料。
    這樣本次 failed.csv 與舊 failed_retry.csv 剛好有同一筆時，
    不會因為本次欄位較少而把舊資料洗成空白。
    """
    merged = dict(existing_row)
    for field in FAILED_FIELDNAMES:
        incoming_value = str(incoming_row.get(field, "") or "")
        if incoming_value:
...
def normalize_and_deduplicate_failed_rows(
    ...
    """
    將 failed rows 正規化，並依 年份 + 頁數 + 筆數 去重。

    回傳順序保留第一次出現的 key；若後面同 key 有較完整資料，
    會合併進原本那一筆。
    """
...
def merge_retry_failed_row_in_memory(failed_row: Dict[str, str]) -> None:
    """
    將一筆資料合併到全域 retry_failed_data，但不立刻寫檔。

    這個函式供 02 啟動時建立工作清單使用，避免讀到舊
    failed_retry.csv 後又被本次 failed.csv 覆蓋掉。
    """
...
def prepare_retry_failed_data(
    ...
    """
    建立 02 本次執行的 failed_retry 工作清單。

    規則：
    1. 先讀取舊 failed_retry.csv，保留以前尚未成功的資料。
    2. 再加入本次 02 的來源資料（通常是本次 01 的 failed.csv）。
    3. 相同 年份 + 頁數 + 筆數 只保留一筆。
    4. 立刻把合併後清單寫回 failed_retry.csv，避免 02 中斷時
       本次新失敗資料尚未留下來。

    重要：
    - 這裡的「合併」只是保留清單，不代表本次一定會 retry 舊資料。
    - main() 後續只會根據 source_rows 建立 groups，因此主流程 04
      仍然只 retry 本次 failed.csv。
    - 05 retry-only 流程的 source_rows 本身就是 failed_retry.csv，
      所以才會 retry 舊資料。
    """
...
def retry_whole_failed_page(
    ...
    """
    處理 failed CSV 裡「筆數 = 0」的整頁 data API 失敗。

    保存順序特別重要：
    1. 先處理整頁所有 detail。
    2. 先把成功 raw 寫入年度 raw、上傳 GCS、重建 all_year。
    3. 成功資料已持久化後，才從 failed_retry 移除已完成項目。

    因此 Cloud Run 若剛好在寫入前中斷，失敗清單仍會留下，
    不會出現「failed_retry 已移除、raw 卻還沒補進去」的遺失狀況。
    """
    ...
    if not isinstance(items, list) or not items:
        record_retry_failed(
            ...
            generation_device="",
            buyer="",
            reason="整頁 retry data API 成功，但本頁 data 為空或格式不是 list，沒有可補抓資料",
        )
        return False, 0
    ...
    resolved_item_keys: List[Tuple[str, int, int]] = []

    # 逐筆呼叫 detail API；仍失敗者會由 retry_item_detail() 立即寫入 failed_retry。
    for row_number, item in enumerate(items, start=1):
        ...
        if item_status == "success":
            ...
            resolved_item_keys.append((year, page, row_number))

        elif item_status == "empty":
            # <ol></ol> 是正常無成交紀錄；不需要保留 retry 項目。
            resolved_item_keys.append((year, page, row_number))

        # item_status == "failed" 時，該筆已由 record_retry_failed() 保留。

    # 先持久化成功 raw。若這裡發生例外，下面的 remove 不會執行，
    # 來源 failed 項目仍會留在 failed_retry.csv，之後可以再補。
    if page_success_rows:
        year_raw_file = find_latest_year_raw_file(year)
        append_rows_to_csv(year_raw_file, page_success_rows)

    # 成功 raw 已上傳（或本頁全都是合法空成交紀錄）後，才移除已完成項目。
    for resolved_year, resolved_page, resolved_row_number in resolved_item_keys:
        remove_retry_failed_row(
            resolved_year,
            resolved_page,
            resolved_row_number,
        )

    # 整頁 data API 已成功取得；page,row=0 的整頁失敗已轉為
    # 個別 detail 的成功 / empty / 單筆 failed 狀態，因此可移除。
...
def main() -> int:
    """
    Cloud Run 02：只 retry 指定來源的失敗資料。

    正常由 04 呼叫時：
        FAILED_CSV_FILE 未設定
        → 只 retry 本次 01 產生的 failed.csv。
        → 舊 failed_retry.csv 會保留，不會被拿來重新 retry，也不會被覆蓋。

    由 05 retry-only 流程呼叫時：
        FAILED_CSV_FILE=trec_direct_transaction_raw_failed_retry.csv
        → 只 retry 歷史 failed_retry.csv。
    """
    ...
    try:
        # 1. /tmp 是新暫存區；先下載年度 raw、all_year、failed、failed_retry。
        storage_client = create_storage_client()
        download_existing_files_from_gcs()

        # 2. 決定本次真正要 retry 的來源。
        #    - 04 正常呼叫：來源是本次 failed.csv。
        #    - 05 retry-only 呼叫：來源是 failed_retry.csv。
        failed_csv = find_failed_csv()
        if failed_csv is None:
            print("找不到 retry 來源 failed CSV。")
            print("仍將本次工作快照完整寫出，讓後續 03 可使用同一個 PIPELINE_DT。")
            upload_all_current_raw_snapshot()
            return 0

        source_rows = normalize_and_deduplicate_failed_rows(read_csv_rows(failed_csv))
        if not source_rows:
            print("retry 來源 failed CSV 只有表頭或沒有可 retry 資料。")
            print("仍將本次工作快照完整寫出，讓後續 03 可使用同一個 PIPELINE_DT。")
            upload_all_current_raw_snapshot()
            return 0
        ...
        if not groups:
            print("retry 來源 failed CSV 裡沒有可 retry 的年份 / 頁數 / 筆數。")
            upload_all_current_raw_snapshot()
            return 0
        ...
        if is_retry_only_source:
            ...
            print("舊 failed_retry.csv 只會保留，不會在本次重抓")

        # 01 已完成時，本次 raw 快照已有完整基礎資料；05 則會先複製一份新的 dt 快照。
        # 02 的 retry 是增量補資料：每次成功補回的年度 raw 與 all_year 都會立即上傳。

        # 3. 建立「保留舊 backlog + 加入本次來源」的 retry 清單。
        #    後續 groups 仍只根據 source_rows，因此不會在主流程意外重抓舊 backlog。
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                for (year, page_number), rows in sorted(
                    ...
                    # 沒有 page,row=0 時，只 retry 來源檔指定的單筆 detail。
                    for row in sorted(
                        single_failed,
                        ...
                        if row_status == "success":
                            # append_rows_to_csv 會先寫 raw / all_year 到 GCS；成功後才移除 retry。
                            year_raw_file = find_latest_year_raw_file(year)
                        ...
                        # failed 狀態已由 record_retry_failed() 即時保留。
        ...
        rebuild_all_year_csv()
        upload_file_to_gcs(
            ALL_YEARS_RAW_CSV_FILE, raw_snapshot_blob_name(ALL_YEARS_RAW_CSV_FILE.name)
        )
        save_retry_failed_csv(force_create_empty=True)
