...
執行環境變數：
    ...
    GCS_PREFIX=raw_data/direct_transaction
    LOCAL_WORKDIR=/tmp
...
RUN_DATE_SHORT = RUN_NOW.strftime("%m%d")
...
def get_year_raw_csv_name(year: str) -> str:
    """年度 raw 檔名，例如 trec_direct_transaction_raw_2026_0620.csv。"""
...
def should_download_from_gcs(filename: str) -> bool:
    """
    Cloud Run 每次 /tmp 是新暫存區，啟動時需下載：
    1. 所有年度 raw：重建 all_year 時保留其他年度。
    2. all_year：保留舊檔供人工查看，雖然後面仍會重建。
    3. status：必須下載才能 append 保留歷史。

    failed.csv 不下載，因為它每次 01 都要重設為「本次失敗清單」。
    """
    return (
        filename == ALL_YEARS_RAW_CSV_NAME
        or filename == STATUS_CSV_NAME
        or is_year_raw_filename(filename)
    )


def download_existing_output_files_from_gcs() -> None:
    """從 GCS 下載既有年度 raw / all_year / status 到 /tmp。"""
    if storage_client is None:
    ...
    for blob in storage_client.list_blobs(GCS_BUCKET, prefix=prefix):
        ...
        # /tmp 正常是空的；若因同一容器重複使用而已有檔案，就不重複下載。
        ...
        # 保留 GCS 更新時間，後續判斷同一年哪一份 raw 最新時比較合理。
        try:
    ...
    print("已下載既有檔案數：", downloaded_count)
...
def upload_output_files_to_gcs() -> None:
    ...
    - failed 本次清單
    """
    candidates: List[Path] = [ALL_YEARS_RAW_CSV_FILE, FAILED_CSV_FILE]
...
def find_latest_year_raw_files() -> Dict[str, Path]:
    """找每個年份最新的一份年度 raw，供 all_year 合併。"""
...
def rebuild_all_year_csv() -> None:
    """將每個年份最新年度 raw 合併為固定 all_year CSV。"""
...
def main() -> int:
    ...
    try:
        # 1. Cloud Run 啟動後先連 GCS，下載保留 all_year 所需的舊檔案。
        storage_client = create_storage_client()
        download_existing_output_files_from_gcs()

        # 2. failed.csv 不保留歷史；本次 01 一開始就重設並上傳空表頭檔。
        initialize_failed_csv_for_new_run()

        # 3. 開首頁，取得 Cookie / CSRF，再使用 API 取得資料。
