...
# 2. 觸發 Cloud Run Job：trec-update-dt-03。
# 3. 完成去重、產生最終 raw、發布資料。
#
...
DAG_ID = "trec_update_dt_03_etl"

ETL_JOB_NAME = "trec-update-dt-03"
...
with DAG(
    dag_id=DAG_ID,
    description="直轉供更新 DAG 03：ETL 去重與最終資料發布",
    ...
    # -----------------------------------------------------
    # 唯一 Task：
    # 執行 Cloud Run Job 03 ETL 去重與發布
    # -----------------------------------------------------
    run_cloud_run_job_03 = BashOperator(
        task_id="run_cloud_run_job_03_etl",
